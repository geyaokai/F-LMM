...
if __name__ == '__main__':
    ...
    parser.add_argument('--log-file', default='eval_refcoco.log', type=str, help='Path to save log file')  # 新增：日志文件参数
    ...
    logging.basicConfig(
        ...
        handlers=[
            logging.FileHandler(args.log_file),  # 日志文件输出
    ...
    logger.info(messages)  # 替换：将accelerator.print改为logger.info
    ...
    if args.checkpoint is not None:
        ...
        logger.info(f"Unexpected parameters: {unexpected}")  # 新增：日志记录未预期参数

    print(f"Start moving model to device: {accelerator.device}", flush=True)
    logger.info(f"Start moving model to device: {accelerator.device}")  # 新增：日志记录模型迁移开始
    tik = time()
    model = model.to(device=accelerator.device)
    print(f"Finished moving model to device: {accelerator.device}, time used: {time() - tik}", flush=True)
    logger.info(f"Finished moving model to device: {accelerator.device}, time used: {time() - tik}")  # 新增：日志记录模型迁移完成
    model.eval()
    ...
    logger.info(f"Do concatenation? {args.concat}")  # 新增：日志记录是否拼接
    ...
    for name, subset in refcoco_subsets.items():
        ...
        logger.info(f"Start evaluating {name}")  # 新增：日志记录开始评估
        ...
        if accelerator.is_main_process:
            ...
            logger.info(f"Collected {len(results)} result samples from all gpus")  # 新增：日志记录收集结果
            ...
            logger.info(f"Evaluation results on {name}: {metrics}")  # 新增：日志记录评估结果

        accelerator.print(f"Finished evaluating {name}")
        logger.info(f"Finished evaluating {name}")  # 新增：日志记录结束评估
