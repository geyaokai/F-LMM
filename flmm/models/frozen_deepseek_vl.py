...
class FrozenDeepseekVLSAM(FrozenDeepseekVL):
    ...
    def _forward(self, data_sample):
        ...
        Args:
            data_sample (dict): 包含以下字段的数据样本
                - 'input_ids': [seq_len] token IDs
                - 'pixel_values': [3, H, W] 预处理后的图像
                - 'mask_ids': [seq_len] 每个 token 对应的 mask 索引
                - 'masks': [num_masks, H', W'] ground truth masks
                - 'meta_data': 图像处理元数据（padding、尺寸等）
                - 'image': PIL.Image 原始图像
        
        Returns:
            dict: 包含以下字段的输出
                - 'pred_masks': [num_masks, H', W'] U-Net 输出的粗粒度 mask
                - 'sam_pred_masks': [num_masks, H_orig, W_orig] SAM 细化后的 mask
                - 'mask_ids': [seq_len] mask 索引
                - 'hidden_states': [seq_len, dim] 加权融合后的 hidden states
                - 'mask_attentions': [num_masks, channels, H', W'] 处理后的注意力图
        """
        # ========== 步骤 1: 准备输入 ==========
        # 获取可学习的层权重（用于加权融合不同层的 hidden states）
        text_layer_weights = self.get_text_layer_weights()
        
        # 准备图像输入: [3, H, W] -> [1, 1, 3, H, W] (batch_size=1, num_images=1)
        pixel_values = data_sample['pixel_values'][None, None].to(
            device=self.deepseek_vl.device,
            dtype=self.deepseek_vl.dtype)
        
        # 准备文本输入: [seq_len] -> [1, seq_len]
        input_ids = data_sample['input_ids'][None].to(self.deepseek_vl.device)
        
        # 找到图像 token 的位置（576 个 <image_placeholder> token）
        images_seq_mask = input_ids == self.image_token_idx
        # images_emb_mask: 标记哪些图像嵌入会被使用 [1, 1, 576]
        images_emb_mask = torch.ones((1, 1, images_seq_mask.sum()), dtype=torch.bool,
                                     device=self.deepseek_vl.device)

        # 准备输入嵌入：将图像和文本 token 转换为嵌入向量
        # 关键：<image_placeholder> token 的嵌入会被替换为实际的图像嵌入
        ...
        # 提取对图像 token 的注意力（只保留文本 token 对图像 token 的注意力）
        # 形状: [num_layers, num_heads, seq_len, num_image_tokens]
        attentions = [attn[0, ..., images_seq_mask[0]] for attn in outputs.attentions]
        
        # 重塑注意力为空间维度: [num_heads, seq_len, num_image_tokens] -> [num_heads, seq_len, 24, 24]
        # DeepSeekVL 固定使用 24×24 = 576 个图像 patch
        attentions = [attn.view(*attn.shape[:-1], self.clip_shape, self.clip_shape) for attn in attentions]
        
        # 提取 hidden states 并加权融合
        # 获取所有语言模型层的 hidden states
        ...
        # 使用可学习的层权重加权融合: [num_layers, seq_len, dim] -> [seq_len, dim]
        ...
        mask_attentions = []  # 每个 mask 的注意力图
        text_embeds = []      # 每个 mask 的文本嵌入
        ...
        for mask_id in range(len(masks)):
            ...
            # 合并所有层的注意力
            # 对于每一层: [num_heads, num_matched_tokens, 24, 24] -> [num_matched_tokens, 24, 24] (merge heads)
            # 然后: 对多个 token 取平均/最大 -> [24, 24]
            # 最后: 拼接所有层 -> [num_layers * num_heads, 24, 24]
        ...
        # 移除 padding: 从 [num_masks, padded_H, padded_W] 提取 [num_masks, H, W]
        pred_masks \
        ...
        # ========== 步骤 7: SAM 细化 mask ==========
        # SAM 使用原始图像、粗粒度 mask 和文本嵌入来生成精细的 mask
        # 输入: PIL.Image, [num_masks, H, W] pred_masks, [num_masks, dim] text_embeds
        # 输出: [num_masks, H_orig, W_orig] 精细 mask（原始图像尺寸）
